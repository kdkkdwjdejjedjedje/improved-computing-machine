...
function buildRelatedCards(relatedChannels) {
  return relatedChannels.map(related => `
          <a class="related-channel-card" href="/canais/${escapeHtml(related.id)}.html" aria-label="Assistir ${escapeHtml(related.name)} online">
            <img src="${escapeHtml(related.logo)}" alt="Logo do canal ${escapeHtml(related.name)}" loading="lazy">
    ...
    <div class="channel-shell">
      <section class="channel-hero" aria-labelledby="channel-title">
        ...
        <div class="channel-heading">
          <img class="channel-logo-main" src="${escapeHtml(logo)}" alt="Logo do canal ${escapeHtml(nome)}" loading="eager">
          <div class="channel-info">
            <p class="channel-kicker">${escapeHtml(categoryLabel)}</p>
            <h1 id="channel-title">${escapeHtml(nome)} ao vivo</h1>
            ${catsBadges ? `<div class="badges-row">${catsBadges}</div>` : ''}
          </div>
        </div>

        <p class="channel-lead">
          Assista <strong>${escapeHtml(nome)} ao vivo online grátis</strong> em alta definição (HD) no Pirate TV. 
          Acompanhe toda a programação do canal ${escapeHtml(context.extra)} direto pelo seu navegador. 
          Sem travamentos e sem necessidade de cadastro.
        </p>
      </section>

      <section class="watch-panel" aria-labelledby="watch-title">
        <div class="watch-panel-head">
          <h2 id="watch-title">${escapeHtml(nome)} - Player 01 (HD)</h2>
        ...
        <div class="channel-actions">
          <a href="/canais">EXPLORAR OUTROS CANAIS</a>
          <a href="/bbb">BBB AO VIVO</a>
        </div>
      </section>

      <article class="channel-seo">
        <h2>Pirate TV Oficial – Assistir ${escapeHtml(nome)} Online Grátis</h2>
        <p>
          O <strong>Pirate TV</strong> traz para você a melhor experiência para assistir <strong>${escapeHtml(nome)} ao vivo</strong> gratuitamente. 
          Nossa plataforma é otimizada para entregar transmissões rápidas e estáveis em qualquer dispositivo, seja no celular, tablet, computador ou Smart TV.
        </p>
        <p>
          ${escapeHtml(nome)} é um dos canais mais buscados na categoria ${escapeHtml(categoryLabel)}, oferecendo conteúdo de qualidade para seus telespectadores. 
          No Pirate TV, você não perde nenhum detalhe da programação e tem acesso instantâneo ao player com apenas um clique.
        </p>
      </article>
      ...
      <section class="related-channels" aria-labelledby="related-title">
        <div class="related-channels-head">
          <div>
            <h2 id="related-title">MULTICANAIS RELACIONADOS</h2>
          </div>
        </div>
  ...
  <footer>
    <div class="footer-inner">
      <div class="footer-brand">
        <img src="../piratetv.png" alt="Pirate TV Logo" class="footer-logo-img" style="height: 44px; width: auto; margin-bottom: 20px;">
        <p class="footer-tagline">Pirate TV: O melhor portal para assistir TV online grátis, futebol ao vivo e multicanais em HD.</p>
      </div>
    ...
    <div class="footer-bottom" style="text-align: center; padding-top: 32px; border-top: 1px solid rgba(255,255,255,0.05); margin-top: 40px; color: #555; font-size: 0.8rem;">
      <p>&copy; 2026 Pirate TV - Todos os direitos reservados.</p>
    </div>
  </footer>
</body>
</html>`;
